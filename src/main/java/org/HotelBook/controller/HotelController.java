...
import org.HotelBook.model.Image;
...
import org.HotelBook.repository.CategoryRepository;
import org.HotelBook.repository.CommentRepository;
import org.HotelBook.repository.HotelRepository;
import org.HotelBook.repository.ImageRepository;
...
public class HotelController {
	...
	{		
		for(Room r : hotels.findById(id).get().getRooms().values())
		{
		...
		hotels.delete(hotels.findById(id).get());
		return "redirect:/hotels";
	} 
	...
	public String approve(@PathVariable("id") long id, Model model) {
		Hotel h = hotels.findById(id).get();
		h.setStatus(true);
		hotels.save(h);
		...
		if(files.length > 0)
		{
			for(int i=0; i < files.length; i++){
				...
				try {
					...
					image.setHotel(hotels.findById(id).get());
	...
	public String deleteImage(@PathVariable("id") long id, @PathVariable("id_image") long id_image, Model model) { 	
		Image image = images.findById(id_image).get();
		images.delete(image);
		return "redirect:/hotels/{id}/upload";
	}
	...
	{		
		...
		model.addAttribute("occupancy", getOccupancy(hotels.findById(id).get(), booking.getBegin_date(), booking.getEnd_date()));
    	return "hotels/map";
	}
